...
const Sidebar = ({ activeView, setActiveView, handleLogout, user, isCollapsed, setIsCollapsed, isDarkMode }) => (
  <aside className={`fixed top-0 left-0 h-screen bg-white dark:bg-gray-900 border-r border-gray-100 dark:border-gray-800 flex flex-col justify-between overflow-hidden z-20 transition-all duration-500 ${isCollapsed ? 'w-20' : 'w-72'}`}>
    <div className="p-0 flex flex-col h-full">
      ...
      <div className={`p-8 flex items-center space-x-4 transition-opacity ${isCollapsed ? 'justify-center' : ''}`}>
        ...
        {!isCollapsed && <span className="text-lg font-black dark:text-white tracking-widest uppercase text-black dark:text-white">Young Tempo</span>}
      </div>

      <div className={`px-6 mb-8 flex items-center space-x-4 transition-all ${isCollapsed ? 'justify-center ml-0' : 'ml-2'}`}>
...
const MainDashboardView = ({ patients, chats, navigate, isDarkMode }) => (
    <div className="space-y-12">
        <div>
            ...
            <p className="text-lg font-bold text-gray-400">Monitoring and supporting your clinical recipients.</p>
        </div>
            ...
            <div className="bg-white dark:bg-gray-900 rounded-[2.5rem] p-8 shadow-sm border border-transparent dark:border-gray-800 group hover:border-primary-500/30 transition-all">
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                    {patients.slice(0, 4).map(p => (
                        <div key={p._id} className="p-5 rounded-[2rem] bg-gray-50 dark:bg-gray-800/50 border border-transparent dark:border-gray-700/50 group/card hover:bg-white dark:hover:bg-gray-800 transition-all cursor-pointer shadow-sm hover:shadow-md">
                            <h3 className="font-black dark:text-white text-md mb-1 truncate tracking-tight">{p.email}</h3>
                            <p className="text-[10px] font-black text-primary-500 uppercase tracking-widest mb-4">{(p.patientDetails?.diagnosis) || "Clinical Tracking"}</p>
                            <button 
                                onClick={async () => {
                    ...
                    {patients.length === 0 && <p className="col-span-full text-center py-8 text-gray-400 font-bold text-sm">No patients assigned</p>}
                </div>
            </div>
        </div>
    </div>
...
const CaretakerDashboard = () => {
    ...
    if (loading) return (
        <div className={`h-screen w-full flex items-center justify-center transition-colors duration-500 ${isDarkMode ? 'bg-black' : 'bg-gray-50'}`}>
    ...
    return (
        <div className={`flex min-h-screen transition-colors duration-500 ${isDarkMode ? 'bg-black' : 'bg-gray-50'} text-gray-900 dark:text-gray-100`}>
            ...
            <div className={`h-screen ${activeView === 'messages' ? 'overflow-hidden' : 'overflow-y-auto'} px-10 flex-1 transition-all duration-300
 ${isCollapsed ? 'ml-20' : 'ml-72'}`}>
                <main className={`flex-1 ${activeView === 'messages' ? 'flex flex-col justify-center' : 'pb-20'}`}>
                    ...
                    {activeView === 'patients' && (
                        <div className="space-y-12">
                            <div>
                                <h1 className="text-6xl font-black dark:text-white tracking-tighter mb-2">Recipient <span className="text-purple-500">Registry</span></h1>
                                <p className="text-lg font-bold text-gray-400">Manage clinical oversight and messaging with your clinical team.</p>
                            </div>
                            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                                {patients.map(p => (
                                    <div key={p._id} className="bg-white dark:bg-gray-900 rounded-[2.5rem] p-8 shadow-sm border border-transparent dark:border-gray-800 hover:border-purple-500/30 transition-all">
                                        ...
                                        <h3 className="text-xl font-black dark:text-white tracking-tight mb-1">{p.email}</h3>
                                        <p className="text-[10px] font-black uppercase tracking-widest text-primary-500 mb-6">{p.phone}</p>
                                        <button 
                                            onClick={async () => {
                                                await chatService.ensureChat([p._id], 'caretaker-patient');
                                                navigate('/chat');
                    ...
                    {activeView === 'reminders' && (
                        <div className="space-y-12">
                             <div>
                                <h1 className="text-6xl font-black dark:text-white tracking-tighter mb-2">Patient <span className="text-yellow-500">Reminders</span></h1>
                                <p className="text-lg font-bold text-gray-400">Schedule daily tasks and medical events for your clinical team.</p>
                            </div>
                            ...
                            <div className="bg-white dark:bg-gray-900 rounded-[2.5rem] p-10 shadow-sm border border-transparent dark:border-gray-800">
                                <div className="grid md:grid-cols-4 gap-6 items-end mb-10">
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <input className="w-full bg-gray-50 dark:bg-gray-800 border-none rounded-2xl p-4 focus:ring-4 focus:ring-primary-500/10 dark:text-white transition-all font-bold" placeholder="e.g. Physical Therapy" value={reminderForm.title} onChange={e => setReminderForm({ ...reminderForm, title: e.target.value })} />
                                    </div>
                                    <div className="space-y-2">
                                        <label className="text-[10px] font-black uppercase tracking-widest text-gray-400 ml-4">Event Date</label>
                                        <input type="date" className="w-full bg-gray-50 dark:bg-gray-800 border-none rounded-2xl p-4 focus:ring-4 focus:ring-primary-500/10 dark:text-white transition-all font-bold" value={reminderForm.date} onChange={e => setReminderForm({ ...reminderForm, date: e.target.value })} />
                                    </div>
                                    <div className="flex gap-4">
                                        <div className="flex-1 space-y-2">
                                            <label className="text-[10px] font-black uppercase tracking-widest text-gray-400 ml-4">Start Time</label>
                                            <input type="time" className="w-full bg-gray-50 dark:bg-gray-800 border-none rounded-2xl p-4 focus:ring-4 focus:ring-primary-500/10 dark:text-white transition-all font-bold" value={reminderForm.time} onChange={e => setReminderForm({ ...reminderForm, time: e.target.value })} />
                                        </div>
                                        <div className="flex flex-col justify-end pb-3">
                                            <label className="flex items-center gap-3 cursor-pointer group">
                                                <input type="checkbox" className="w-5 h-5 rounded-lg border-2 border-gray-100 dark:border-gray-700 text-primary-500 focus:ring-0 transition-all pointer-events-none" checked={reminderForm.isRecurring} onChange={e => setReminderForm({ ...reminderForm, isRecurring: e.target.checked })} />
                                                <span className="text-[10px] font-black uppercase tracking-widest text-gray-400 group-hover:text-primary-500 transition-colors">Daily</span>
                                            </label>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="flex justify-between items-center mb-6">
                                    <span className="text-sm font-black dark:text-white uppercase tracking-wider">{reminders.length} Active Events</span>
                                    <button onClick={createReminder} className="flex items-center gap-2 px-8 py-4 bg-primary-500 text-white rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-primary-600 transition-all shadow-xl shadow-primary-500/20 active:scale-95">
                                ...
                                <div className="space-y-3 max-h-[500px] overflow-auto custom-scrollbar pr-2">
                                    {reminders.map(r => (
                                        <div key={r._id} className="p-6 rounded-[2rem] bg-gray-50 dark:bg-gray-800/50 border border-transparent dark:border-gray-700/50 flex items-center justify-between group hover:bg-white dark:hover:bg-gray-800 transition-all shadow-sm">
                                            <div className="flex items-center gap-6">
                                                <div className={`w-12 h-12 rounded-2xl ${r.isRecurring ? 'bg-yellow-500' : 'bg-primary-500'} flex items-center justify-center text-white shadow-lg`}>
                                                    {r.isRecurring ? <Calendar size={20} /> : <Bell size={20} />}
                                                </div>
                                                <div>
                                                    <h3 className="text-md font-black dark:text-white tracking-tight uppercase">{r.title}</h3>
                                                    <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">{r.text || "Daily Medical Activity"}</p>
                                                </div>
                                            </div>
                                            <div className="text-right">
                                                <div className="text-sm font-black dark:text-white tracking-tighter">{r.time}</div>
                                                <div className="text-[9px] font-black uppercase tracking-widest text-primary-500">{new Date(r.date).toLocaleDateString([], { day: '2-digit', month: 'short' })}</div>
                                            </div>
                                        </div>
                                    ))}
                                    {reminders.length === 0 && (
                                        <div className="text-center py-20 bg-gray-50 dark:bg-gray-800/20 rounded-[2rem] border border-dashed border-gray-200 dark:border-gray-800">
                                            ...
                                            <p className="text-gray-400 dark:text-gray-600 font-bold text-sm uppercase tracking-widest text-center mx-auto">No events scheduled</p>
                                        </div>
                                    )}
                                </div>
                            </div>
